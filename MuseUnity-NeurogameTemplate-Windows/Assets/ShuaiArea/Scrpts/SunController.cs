...
public class SunController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (sunLight != null && InteraxonInterfacer.Instance != null && !isStateTwo)
        {
            // 只在状态一时根据flow值映射光照强度
            float mappedIntensity = Mathf.Lerp(minIntensity, maxIntensity, Mathf.Clamp(InteraxonInterfacer.Instance.flow / 2f, 0f, 1f));
